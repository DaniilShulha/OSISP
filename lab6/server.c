...
int main() {
    ...
    if (server_socket == -1) {
        perror("Socket error creation occured");
        exit(1);
    }
    ...
    if (bind(server_socket, (struct sockaddr *)&server_addr,
        ...
        perror("Error during socket and adress connection");
        exit(1);
    }
    ...
    printf("Server launched. Waiting for new connections...\n");
    ...
    while (1) {
        ...
        if (client_socket == -1) {
            perror("Connection error during accept");
            continue;
        }

        char username[20];
        if (recv(client_socket, username, sizeof(username), 0) <= 0) {
            perror("Error during getting username from client");
            close(client_socket);
            continue;
        }
        ...
        if (num_clients < MAX_CLIENTS) {
            ...
            for (size_t i = 0; i < num_clients; i++) {
                if (strcmp(clients[i].username, client->username) == 0) {
                    char reject_msg[100] = "Username is not unique\n";
            ...
            printf("Max client limit reached. Client %s ""denied\n",client->username);
            char reject_msg[100] = "Too many connections\n";
